...
export function registerConnectionHandlers({
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, configPayload: unknown) => {
      ...
      return runConnectionOperation('connect', async () => {
        ...
        try {
          ...
          const fullConfig = storedServers.find(
          ...
          if (
            ...
            logger.info(
              'IPC',
              'connect skipped: already connected to selected server',
              {
            ...
            if (process.platform === 'win32') {
              ...
              if (relaunched) {
                sendToRenderer(
                  ...
                  'Restarting UltimaVLESS with Administrator rights...',
                );
                deps.app.releaseSingleInstanceLock();
                deps.app.quit();
                return {
                  ok: false as const,
                  error: 'Restarting as administrator',
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent) => {
      ...
      return runConnectionOperation('disconnect', async () => {
        ...
        try {
          deps.configService.clearPendingTunReconnect();
          await deps.connectionStackService.resetNetworkingStack({
            stopXray: true,
          });
