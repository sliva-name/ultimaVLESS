...
export function normalizeAddSubscriptionPayload(
  payload: unknown,
): AddSubscriptionPayload {
  ...
  if (name.length > MAX_SUBSCRIPTION_NAME_LENGTH) {
    throw new Error(
      `Subscription name is too long (max ${MAX_SUBSCRIPTION_NAME_LENGTH} characters)`,
    );
  }
  if (!url) throw new Error('Subscription URL is required');
  if (url.length > MAX_SUBSCRIPTION_URL_LENGTH) {
    throw new Error(
      `Subscription URL is too long (max ${MAX_SUBSCRIPTION_URL_LENGTH} characters)`,
    );
  }
  return { name, url };
}
...
export function normalizeManualLinks(payload: unknown): string {
  if (typeof payload === 'string') {
    if (payload.length > MAX_MANUAL_LINKS_LENGTH) {
      throw new Error(
        `Manual links payload is too large (max ${MAX_MANUAL_LINKS_LENGTH} characters)`,
      );
    }
    return payload.trim();
  }
