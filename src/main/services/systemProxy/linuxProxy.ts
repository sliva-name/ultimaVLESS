...
export class LinuxProxyAdapter {
  ...
  async enable(httpPort: number, socksPort: number): Promise<void> {
    ...
    try {
      await this.runGsettings(['set', 'org.gnome.system.proxy', 'mode', 'manual']);
      await this.runGsettings(['set', 'org.gnome.system.proxy.http', 'host', '127.0.0.1']);
      await this.runGsettings(['set', 'org.gnome.system.proxy.http', 'port', String(httpPort)]);
      await this.runGsettings(['set', 'org.gnome.system.proxy.https', 'host', '127.0.0.1']);
      await this.runGsettings(['set', 'org.gnome.system.proxy.https', 'port', String(httpPort)]);
      await this.runGsettings(['set', 'org.gnome.system.proxy.socks', 'host', '127.0.0.1']);
      await this.runGsettings(['set', 'org.gnome.system.proxy.socks', 'port', String(socksPort)]);
      logger.info('SystemProxyService', 'Linux proxy enabled via gsettings', { httpPort, socksPort });
    } catch (error) {
  ...
  async disable(): Promise<void> {
    await this.ensureSupported();
    try {
      await this.runGsettings(['set', 'org.gnome.system.proxy', 'mode', 'none']);
  ...
  async restoreState(snapshot: LinuxProxySnapshot): Promise<void> {
    ...
    await this.runGsettings(['set', 'org.gnome.system.proxy.http', 'host', snapshot.httpHost]);
    await this.runGsettings(['set', 'org.gnome.system.proxy.http', 'port', String(snapshot.httpPort)]);
    await this.runGsettings(['set', 'org.gnome.system.proxy.https', 'host', snapshot.httpsHost]);
    await this.runGsettings(['set', 'org.gnome.system.proxy.https', 'port', String(snapshot.httpsPort)]);
    await this.runGsettings(['set', 'org.gnome.system.proxy.socks', 'host', snapshot.socksHost]);
    await this.runGsettings(['set', 'org.gnome.system.proxy.socks', 'port', String(snapshot.socksPort)]);
    await this.runGsettings(['set', 'org.gnome.system.proxy', 'mode', snapshot.mode]);
  }
  ...
  private async getBackend(): Promise<Backend> {
    ...
    try {
      await runCommand('gsettings', ['writable', 'org.gnome.system.proxy', 'mode'], LINUX_TIMEOUT_MS);
