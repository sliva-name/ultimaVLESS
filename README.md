...
| **Hysteria 2** | Ссылка `hy2://` / `hysteria2://` или JSON-подписка |
| **WireGuard** | Только JSON-подписка (готовый outbound Xray) |
| **VMess** | Только JSON-подписка (готовый конфиг Xray) |

Reality и Vision — это не отдельные протоколы, а варианты настройки VLESS.

**Не поддерживаются:** TUIC, ссылки `vmess://` и другие протоколы вне Xray.

## Что умеет программа

- Импорт списка серверов по **ссылке на подписку** (текст, Base64 или JSON)
- Добавление серверов **вручную** — можно вставить текст с ссылками `vless://`, `trojan://`, `ss://`, `hy2://` / `hysteria2://`
